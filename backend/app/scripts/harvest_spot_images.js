...
/* Commons' imageinfo reports thumbwidth as the width you ASKED for, and then
 * serves whatever bucketed render it already had - ask for 1600 and you are
 * handed 1920, or the untouched original when it is smaller than the request.
 * Trusting the API cost 2,311 of 2,341 rows a wrong width and height on the
 * first pass, with the aspect ratio right and the absolute numbers meaningless.
 *
 * So dimensions come off the bytes. Header parsing rather than a library,
 * because this file installs nothing.
 */
function readDimensions(buf) {
  ...
  if (buf.length > 4 && buf[0] === 0xff && buf[1] === 0xd8) {
    ...
    while (i < buf.length - 9) {
      ...
      /* SOF0-SOF15 carry the frame size. C4, C8 and CC are Huffman/arithmetic
       * tables that share the range and must be skipped, not read. */
  ...
  for (const spot of spots) {
    ...
    rec.images.forEach((img, i) => {
      ...
      if (dims) {
        ...
        img.dimensions_from = 'file';
      } else {
        img.dimensions_from = 'commons-api (unparsed header)';
      }
      placed += 1;
    });
